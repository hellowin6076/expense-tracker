...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        // URL 경로에 따라 모드 설정
        const mode = location.pathname === '/huimo' ? 'remittance' : 'kakeibo';
        setAppMode(mode);
  ...
  const handleLogin = async () => {
    try {
      ...
      // Auth 상태는 onAuthStateChanged에서 처리됨
      const mode = location.pathname === '/huimo' ? 'remittance' : 'kakeibo';
      sessionStorage.setItem('appMode', mode);
      setAppMode(mode);
  ...
  const getAvailableMonths = () => {
    if (appMode === 'remittance') {
      ...
      remittances.forEach(rem => {
        const month = rem.date.substring(0, 7);
        monthSet.add(month);
      });
  ...
  const getTrendData = () => {
    ...
    return months.map(payrollMonth => {
      ...
      const himoTotal = himoNormalTotal + himoTatekaeTotal;
      const azuTotal = azuNormalTotal + azuTatekaeTotal;
      const total = himoNormalTotal + azuNormalTotal;
    ...
    return (
      <div className="login-container">
        <div className="login-box">
          <h1>🔐 {location.pathname === '/huimo' ? '送金管理' : '支出明細'}</h1>
          <p className="firebase-badge">🔥 Firebase版 - 給料日精算 (21日〜20日)</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginTop: '30px' }}>
            <button
              onClick={handleLogin}
              className="login-button"
              style={{ backgroundColor: location.pathname === '/huimo' ? '#E91E63' : '#667eea' }}
            >
              <span style={{ fontSize: '20px', marginRight: '8px' }}>
                {location.pathname === '/huimo' ? '💸' : '💰'}
              </span>
              Googleでログイン
            </button>
          </div>
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/huimo" element={<AppContent />} />
      </Routes>
    </BrowserRouter>
  );
}
